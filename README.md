...
| **Auto-resolve + incident record** *(extension)* | Any restock (chat, alert button, or card) **auto-resolves** the related `OVERSELL_ATTEMPT` incidents for that item/org. The incident **row is never deleted** — its `id` is preserved and stamped with a `resolution` audit note + `resolved_at`, so you keep a permanent record of what fired and how it was closed. Velocity/shrinkage incidents stay open (different root cause) |
